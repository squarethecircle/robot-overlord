import {octokit, PullRequest, getParamsForPR, getActionUsername} from './octokit';
...
const currentPRApprovals = async (pullRequest: PullRequest): Promise<string[]> => {
    const reviews = await octokit.paginate(octokit.rest.pulls.listReviews, getParamsForPR(pullRequest));
    if (!reviews) {
        return [];
    }
...
const getCodeownerApprovalStatusForPR = async (pullRequest: PullRequest): Promise<[CodeownersBotAction, CodeownersStatus[]]> => {
    ...
    const statuses: CodeownersStatus[] = requiredApprovals.map(requirement => ({
      requirement,
      satisfiedBy: [...new Set(requirement.members.map(owner => ownerMatchedBy(owner, currentApprovals)).flat())]
    ...
    if (hasAtLeastOneApproval && passesAllOwnersRequirements) {
        finalAction = alreadyApprovedByBot ? CodeownersBotAction.NOTHING : CodeownersBotAction.APPROVE;
    } else {
        finalAction = alreadyApprovedByBot ? CodeownersBotAction.REQUEST_CHANGES : CodeownersBotAction.COMMENT;
    }

    return [finalAction, statuses];
};

const generateReviewComment = (statuses: CodeownersStatus[], pr: PullRequest): string => {
    const usersToConsider = new Set((pr.assignees || []).concat([pr.user]).filter(tg.isNotNullish).map(user => user.login));
    const membersFormat = (members: string[]) => {
        const filteredMembers = members.filter(m => usersToConsider.has(m));
        return filteredMembers.length ? `(${filteredMembers.join(', ')})` : '(no reviewers currently assigned)';
    };
    const groupFormat = (owner: CodeownerGroup) => `@${owner.groupName} ${membersFormat(owner.members)}`;
    const userFormat = (owner: CodeownerUser) => `@${owner.username}`;
    const ownerFormat = (owner: Codeowner) => ('members' in owner) ? groupFormat(owner) : userFormat(owner);


    const remainingApprovalOwners = statuses.filter(s => s.satisfiedBy.length === 0).map(s => s.requirement.members).flat();
    ...
    const requirementFormat = (req: CodeownerRequirement) => `- ${req.pattern}: [${req.members.map(ownerFormat).join(' ')}]`;
...
const postReviewComment = async (pullRequest: PullRequest, body: string): Promise<void> => {
    await octokit.rest.issues.createComment({body, ...getParamsForPR(pullRequest)});
};
const postReviewApproval = async (pullRequest: PullRequest, action: 'APPROVE' | 'REQUEST_CHANGES', body: string): Promise<void> => {
    await octokit.rest.pulls.createReview({body, event: action, ...getParamsForPR(pullRequest)});
};

export const onPullRequestUpdate = async (pullRequest: PullRequest): Promise<void> => {
